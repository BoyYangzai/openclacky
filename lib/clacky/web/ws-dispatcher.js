  ...
  switch (ev.type) {
    ...
    case "input_queue": {
      ...
      for (const entry of ev.entries || []) {
        ...
        const guide = iconButton("chat.input.steerMode", '<path d="M4 12h16M14 6l6 6-6 6"/>');
        guide.disabled = !entry.steer_target || entry.delivery === "steer" || entry.content.trimStart().startsWith("/");
        guide.title = I18n.t(entry.steer_target ? "chat.input.steerDescription" : "chat.input.guidanceClosed");
        guide.onclick = () => {
          if (!WS.ready) return;
          guide.disabled = true;
          WS.send({ type: "steer_pending_input", session_id: ev.session_id, id: entry.id, task_id: entry.steer_target });
        };
