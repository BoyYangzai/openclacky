// Creator Center - full-page workspace mounted from the sidebar.
...
(function () {
  const STUDIO_I18N = {
    en: {
      "nav.entry": "Creator Center",
      "ws.title": "Creator Center",
      ...
      "btn.publishBrand": "Publish to brand",
      ...
      "published.brandTitle": "Your brand extensions",
      "published.brandEmpty": "You haven't published any brand extensions yet.",
      "btn.unpublish": "Unpublish",
      "published.confirm": "Unpublish {{id}} from the marketplace?",
      "published.brandConfirm": "Unpublish {{id}} from your brand?",
      ...
      "pub.brand.title.new": "Publish to brand",
      "pub.brand.title.update": "Publish new version to brand",
      "pub.brand.intro": "\"{{name}}\" will be packed and uploaded to your brand's extension library, where only your brand's users can install it.",
      ...
      "extlist.section.cloud": "Marketplace Extensions",
      "extlist.section.cloudHint": "Live on the marketplace",
      "extlist.section.brand": "Brand Extensions",
      "extlist.section.brandHint": "Published to your brand",
      ...
      "extlist.brand.empty": "No brand extensions published yet.",
      ...
      "extlist.btn.publishBrand": "Publish to Brand",
      "extlist.btn.update": "Update",
      "extlist.btn.updateBrand": "Update Brand Version",
      ...
      "extlist.unpublishBrand.confirm": "Unpublish {{id}} from your brand?",
    ...
    zh: {
      ...
      "published.brandTitle": "你发布到品牌的扩展",
      "published.brandEmpty": "你还没有发布到品牌的扩展。",
      "btn.unpublish": "下架",
      "published.confirm": "确定要从市场下架 {{id}} 吗？",
      "published.brandConfirm": "确定要从品牌下架 {{id}} 吗？",
      "err.generic": "出错了：{{msg}}",
      ...
      "pub.brand.title.new": "发布到品牌",
      "pub.brand.title.update": "发布新版本到品牌",
      "pub.brand.intro": "「{{name}}」将被打包并上传到你的品牌扩展库，仅品牌下的用户可安装使用。",
      ...
      "extlist.btn.publishBrand": "发布到品牌",
      "extlist.btn.update": "更新到市场",
      "extlist.btn.updateBrand": "更新到品牌",
      ...
      "extlist.unpublishBrand.confirm": "确定要从品牌下架 {{id}} 吗？",
      ...
      "skills.section.cloud": "云端技能",
      "skills.section.cloudHint": "已发布到平台",
      "skills.section.local": "本地技能",
      "skills.section.localHint": "可发布",
      "skills.cloud.empty": "还没有发布任何技能。",
      "skills.cloud.locked": "成为创作者后才能上传并发布技能。",
      "skills.local.empty": "所有本地技能都已发布。",
      ...
      "skills.iterate.prompt": "更新技能：",
      "skills.shadow.label": "本地覆盖",
      "skills.shadow.tooltip": "本地副本覆盖了同名品牌技能",
      "skills.newSkill.label": "用 /skill-creator 创建新技能",
      "skills.newSkill.btn": "创建新技能",
      "skills.promo.text": "发布你的技能，打造自己的品牌。",
      "skills.promo.link": "了解更多 →",
      "skills.locked": "发布云端技能需要创作者授权。",
  ...
  function toast(msg, type) {
    if (window.Clacky && Clacky.Modal && Clacky.Modal.toast) Clacky.Modal.toast(msg, type || "info");
  }
  ...
  function openModal({ title, body, buttons, onClose }) {
    const overlay = el("div", { class: "modal-overlay studio-modal" });
    const box = el("div", { class: "modal-box sm" });
    const header = el("div", { class: "modal-header" });
    header.appendChild(el("h3", { class: "modal-title", text: title }));
    const close = () => { if (overlay.parentNode) overlay.parentNode.removeChild(overlay); };
    const closeBtn = el("button", {
      class: "modal-close-btn", text: "×", "aria-label": t("modal.close"),
      onclick: () => { close(); if (onClose) onClose(); },
    });
  ...
  function runBindingFlow() {
    return new Promise((resolve) => {
      ...
      const modal = openModal({
        ...
        onClose: () => { polling = false; resolve(false); },
  ...
  function runPublishFlow(ext, prevVersionOrPromise, { onPublishDone, origin } = {}) {
    return new Promise((resolve) => {
      const isBrand = origin === "self";
      ...
      const bodyChildren = [
        el("p", { class: "studio-modal-intro", text: introText }),
        verField,
        notesField,
        status,
      ];
  ...
  function showRenameForm(ext, serverError) {
    ...
    async function submit() {
      ...
      cancelBtn.disabled = true;
      modal.overlay.querySelector(".modal-close-btn").disabled = true;
      input.disabled = true;
      status.style.display = "none";
  ...
  function openMetaEditModal(ext, onSaved) {
    ...
    const modal = el("div", { class: "modal-box sm" });
    ...
    const header = el("div", { class: "modal-header" });
    header.appendChild(el("h3", { class: "modal-title", text: t("meta.edit.title") }));
    const closeBtn = el("button", { class: "modal-close-btn", text: "×", "aria-label": t("modal.close") });
    ...
    const footer = el("div", { class: "modal-footer" });
    const cancelBtn = el("button", { class: "btn-secondary", text: t("pub.btn.cancel"), onclick: close });
    const saveBtn = el("button", { class: "btn-primary", text: t("meta.save") });
  ...
  function confirmReadme(ext, { onSaved } = {}) {
    if (ext.readme && ext.readme.trim()) return Promise.resolve(true);
    return new Promise((resolve) => {
      const overlay = el("div", { class: "modal-overlay studio-modal" });
      const box = el("div", { class: "modal-box sm" });
      const header = el("div", { class: "modal-header" });
      header.appendChild(el("h3", { class: "modal-title", text: t("pub.readme.section") }));
      const closeBtn = el("button", { class: "modal-close-btn", text: "×", "aria-label": t("modal.close") });
      closeBtn.addEventListener("click", () => close(false, true));
      header.appendChild(closeBtn);
      box.appendChild(header);
      box.appendChild(el("p", { style: "margin-bottom:20px;color:var(--color-text-secondary);font-size:14px", text: t("pub.readme.required") }));
      const footer = el("div", { class: "modal-footer" });
      const skipBtn = el("button", { class: "btn-secondary", text: t("pub.readme.required.skip") });
      const editBtn = el("button", { class: "btn-primary", text: t("pub.readme.required.edit") });
      function close(goEdit, cancelled) {
  ...
  function openReadmeEditModal(extId, initialReadme, onSaved, onCancel) {
    // Use a wider full-overlay modal with a split editor/preview layout.
    const overlay = el("div", { class: "modal-overlay studio-modal" });
    const box = el("div", { class: "modal-box studio-readme-modal" });
    overlay.appendChild(box);
    document.body.appendChild(overlay);
    function close() { if (overlay.parentNode) overlay.parentNode.removeChild(overlay); }

    const header = el("div", { class: "modal-header" });
    header.appendChild(el("h3", { class: "modal-title", text: t("pub.readme.edit.title") }));
    const closeBtn = el("button", { class: "modal-close-btn", text: "×", "aria-label": t("modal.close") });
    ...
    const footer = el("div", { class: "modal-footer" });
    const cancelBtn = el("button", { class: "btn-secondary", text: t("pub.btn.cancel") });
    cancelBtn.addEventListener("click", () => { close(); if (onCancel) onCancel(); });
    const saveBtn = el("button", { class: "btn-primary", text: t("pub.readme.save") });
    saveBtn.addEventListener("click", async () => {
  ...
  function createPublishPanel() {
    ...
    async function doPublish() {
      const ext = store.selected();
      if (!ext) return;
      if (!(await confirmReadme(ext, { onSaved: () => rebuild() }))) return;
      ...
      if (ok) {
        toast(t("pub.success"), "success");
        loadPublished();
      }
    }

    async function doPublishBrand() {
      const ext = store.selected();
      if (!ext) return;
      if (!(await confirmReadme(ext, { onSaved: () => rebuild() }))) return;
      const prevVersionPromise = getJson("/published_brand")
        .then((data) => {
          const match = (data.extensions || []).find((e) => e.id === ext.id);
          return match ? match.version : null;
        })
        .catch(() => null);
      const ok = await runPublishFlow(ext, prevVersionPromise, { onPublishDone: loadPublished, origin: "self" });
      if (ok) {
        toast(t("pub.success"), "success");
        loadPublished();
      }
    ...
    async function loadPublished() {
      ...
      if (!box) return;
      const brandOwner = isBrandOwner();
      const titleKey = brandOwner ? "published.brandTitle" : "published.title";
      const emptyKey = brandOwner ? "published.brandEmpty" : "published.empty";
      const confirmKey = brandOwner ? "published.brandConfirm" : "published.confirm";
      box.innerHTML = "";
      box.appendChild(el("div", { class: "studio-label", text: t(titleKey) }));
      ...
      try {
        ...
        box.innerHTML = "";
        box.appendChild(el("div", { class: "studio-label", text: t(titleKey) }));
        const exts = brandOwner
          ? (data.extensions || [])
          : (data.extensions || []).filter((e) => e.origin !== "self");
        if (!exts.length) { box.appendChild(el("p", { class: "studio-empty", text: t(emptyKey) })); return; }
        exts.forEach((e) => {
          ...
          if (!brandOwner || e.origin === "self") {
            ...
            unpubBtn.addEventListener("click", async () => {
              if (!await Clacky.Modal.confirm(t(confirmKey, { id: e.id }))) return;
    ...
    function rebuild() {
      ...
      const bar = el("div", { class: "studio-actions" });
      if (isBrandOwner()) {
        bar.appendChild(el("button", { class: "studio-btn studio-btn-primary studio-publish-brand-btn", text: t("btn.publishBrand"), onclick: doPublishBrand }));
      } else {
  ...
  // Cloud / local (plus brand, for extensions) lists as a segmented control
  // instead of stacked sections — one list visible at a time. `state.active`
  // lives with the caller so the selection survives a rebuild.
  function sectionTabs(sections, state) {
    const bar = el("div", { class: "studio-seg-bar" });
    const seg = el("div", { class: "segmented-control" });
    const hint = el("span", { class: "studio-seg-hint" });
    const listBox = el("div", { class: "studio-seg-body" });
    bar.appendChild(seg);
    bar.appendChild(hint);

    if (!sections.some((s) => s.id === state.active)) state.active = sections[0].id;

    function paint() {
      const current = sections.filter((s) => s.id === state.active)[0] || sections[0];
      seg.innerHTML = "";
      sections.forEach((s) => {
        const btn = el("button", {
          class: "segmented-control__btn" + (s.id === current.id ? " active" : ""),
          text: s.label,
        });
      ...
      if (!current.items.length) {
        listBox.appendChild(el("p", { class: "studio-empty", text: t(current.emptyKey) }));
      } else {
        current.items.forEach((item) => listBox.appendChild(current.cardFn(item)));
      }
    }
    paint();
  ...
  function createExtensionsPanel() {
    ...
    let marketCloud = [];
    let brandPublished = [];
    const sectionState = { active: null };
    ...
    async function reload() {
      ...
      if (typeof Brand !== "undefined" && Brand.userLicensed) {
        try {
          const data = await getJson("/published_brand");
          brandCloud = data.extensions || [];
        } catch (_e) {
          brandCloud = [];
        }
      } else {
      ...
      marketCloud = cloud.filter((e) => e.origin !== "self");
    ...
    function cloudCard(ext) {
      ...
      if (canUnpublish) {
        ...
        un.addEventListener("click", async () => {
          const confirmKey = ext.origin === "self" ? "extlist.unpublishBrand.confirm" : "extlist.unpublish.confirm";
          if (!await Clacky.Modal.confirm(t(confirmKey, { id: ext.id }))) return;
          un.disabled = true;
    ...
    function localCard(ext) {
      ...
      const marketEntry = marketCloud.find((c) => c.id === ext.id);
      const brandEntry = brandCloud.find((c) => c.id === ext.id);
      const published = brandOwner ? !!brandEntry : !!marketEntry;
      const publishedAnywhere = !!cloudEntry;
      const marketPublished = !!marketEntry;
      const card = el("div", { class: "studio-skill-card" });
      const head = el("div", { class: "studio-skill-head" });
      head.appendChild(el("span", { class: "studio-skill-name", text: `${ext.name} (${ext.id})` }));
      ...
      if (brandOwner) {
        ...
        const brandNonAdmin = typeof Brand !== "undefined" && Brand.branded && !Brand.userLicensed;
        const pub = el("button", { class: "studio-btn studio-btn-primary", text: marketPublished ? t("extlist.btn.update") : t("extlist.btn.publish") });
        ...
        pub.addEventListener("click", () => doPublish(ext, marketEntry ? marketEntry.version : null));
    ...
    async function doPublish(ext, prevVersion, origin) {
      if (!(await confirmReadme(ext, { onSaved: () => { reload().then(rebuild); } }))) return;
      const onDone = async () => { await reload(); rebuild(); };
      const ok = await runPublishFlow(ext, prevVersion, { onPublishDone: onDone, origin });
    ...
    function rebuild(headerActions) {
      ...
      const sections = [{
        id: "local",
        label: t("extlist.section.local"),
        hint: t("extlist.section.localHint"),
        items: store.extensions,
        cardFn: localCard,
        emptyKey: "extlist.local.empty",
      }];
      if (!isBrandOwner()) {
        sections.push({ id: "cloud", label: t("extlist.section.cloud"), hint: t("extlist.section.cloudHint"), items: marketCloud, cardFn: cloudCard, emptyKey: "extlist.cloud.empty" });
      }
      if (brandPublished.length > 0 || isBrandOwner()) {
        sections.push({ id: "brand", label: t("extlist.section.brand"), hint: t("extlist.section.brandHint"), items: brandPublished, cardFn: cloudCard, emptyKey: "extlist.brand.empty" });
      }
      sectionTabs(sections, sectionState).forEach((node) => container.appendChild(node));
    }
  ...
  function createSkillsPanel() {
    ...
    function rebuild(headerActions) {
      ...
      const sections = [{
        id: "local",
        label: t("skills.section.local"),
        hint: t("skills.section.localHint"),
        items: local,
        cardFn: localCard,
        emptyKey: "skills.local.empty",
      }, {
        id: "cloud",
        label: t("skills.section.cloud"),
        hint: t("skills.section.cloudHint"),
        items: cloud,
        cardFn: cloudCard,
        emptyKey: locked ? "skills.cloud.locked" : "skills.cloud.empty",
      }];
      sectionTabs(sections, sectionState).forEach((node) => container.appendChild(node));
    }
  ...
  style.textContent = `
    ...
    .studio-seg-bar { display: flex; align-items: center; gap: 12px; margin-bottom: 14px; }
    .studio-seg-hint { font-size: 11px; color: var(--color-text-muted); }
    ...
    .studio-skill-badge { font-size: 11px; padding: 1px 8px; border-radius: 4px; }
    ...
    .studio-modal .modal-header { border-bottom: none; }
