...
RSpec.describe Clacky::MessageFormat::OpenAIResponses do
  ...
  describe ".parse_response" do
    ...
    it "extracts reasoning from an OpenAI official reasoning block inside a message item" do
      data = {
        "status" => "completed",
        "output" => [
          {
    ...
    it "extracts reasoning from a DeepSeek-style top-level reasoning item" do
      data = {
        "status" => "completed",
        "output" => [
          { "type" => "reasoning", "content" => [{ "type" => "reasoning_text", "text" => "Let me think." }] },
    ...
    it "falls back to summary text when reasoning content blocks are absent" do
      data = {
        "status" => "completed",
        "output" => [
          {
      ...
      result = described_class.parse_response(data)
      expect(result[:reasoning_content]).to eq("Brief thought.")
    end
